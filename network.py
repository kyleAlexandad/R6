...
def encode_state(state):
    ...
    In this example, we use:
      - The number of nodes (normalized).
      - The total length of the string representations of all nodes (normalized).
    """
...
class AlphaZeroNet(nn.Module):
    """
    A neural network with two heads:
      - The policy head outputs logits over a fixed maximum action space.
      - The value head outputs a scalar estimating the expected reward.
    """
