...
def self_play_episode(initial_state, target, net, max_actions, num_simulations, max_depth, allowed_creations):
    """
    Runs one self-play episode using MCTS.
    Returns:
      - trajectory: list of (encoded state, target policy distribution) pairs.
      - final_reward: computed via compute_reward.
      - final_state: the resulting state.
    """
    trajectory = []
    state = initial_state
    while not is_terminal(state, target) and len(state.history) < max_depth:
        valid_actions = get_possible_actions(state, allowed_creations)
        if not valid_actions:
            break
        best_action, root = run_mcts(state, target, net, max_actions, num_simulations, max_depth, allowed_creations)
...
def update_network(net, optimizer, trajectory, reward, loss_weight=1.0):
    """
    Updates the network using the self-play trajectory.
